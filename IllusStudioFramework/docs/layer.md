...
| **API** | `addLayer` / `removeLayer` / `setActiveLayer` / opacity+visibility / `duplicateLayer` / `moveLayer` / `mergeLayerDown` / blend getters-setters (planned) |
...
| **v1 out** | Layer groups, masks; engine may ship Normal first and add the catalog below incrementally |
...
- Layers panel shows a short blend badge (e.g. **`N`**) next to the name; tap/menu changes mode.
...
- `enum class BlendMode` — unique cases for every row above (do not overload enum on colliding UI letters).
- `setLayerBlendMode(layerId, mode)` / `layerBlendMode(layerId)`.
- Composite (`SoftwareRenderer` for export / self-check; GPU `LayerCompositor` on present — Normal; other modes T0-12).
